...
async function getClass(id: number | string): Promise<any> {
  let _class = await Class.findByPk(id, {
    attributes: ['name'],
  });

  if (!_class) throw Error('404');
...
async function updateClass(id: number | string, name: string): Promise<void> {
  let _class = await Class.findByPk(id);
  if (!_class) throw Error('404');
...
async function deleteClass(id: number | string): Promise<void> {
  let _class = await Class.findByPk(id);
  if (!_class) throw Error('404');
...
async function getStudents(id: number | string): Promise<any[]> {
...
async function addStudent(
  ...
  if (!_class || !student) throw Error('404');
